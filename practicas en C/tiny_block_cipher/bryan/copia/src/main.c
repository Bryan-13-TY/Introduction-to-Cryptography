...
int main(int argc, char const *argv[]) {
    ...
    do {
        ...
        switch (option) {
            case 1:
                block_status = sbox_generator();
                if (BLOCK_OK == block_status) printf("\n>>> La S-Box se guardo correctamente");
                if (BLOCK_SBOX_MEMORY_ERROR == block_status) printf("\n>>> Hubo un error al reservar memoria para la S-Box");
                if (BLOCK_SBOX_GENERATION_ERROR == block_status) printf("\n>>> Hubo un error al generar y guardar la S-Box");
                wait_key();
                break;
            case 2:
                block_status = secret_key_generator();
                if (BLOCK_OK == block_status) printf("\n>>> La llave secreta se guardo correctamente");
                if (BLOCK_KEY_GENERATION_ERROR == block_status) printf("\n>>> Hubo un error al generar y guardar la llave");
                wait_key();
                break;
            case 3:
                block_status = pbox_generator();
                if (BLOCK_OK == block_status) printf("\n>>> La P-Box se guardo correctamente");
                if (BLOCK_PBOX_GENERATION_ERROR == block_status) printf("\n>>> Hubo un error al generar y guardar la P-Box");
                wait_key();
                break;
            case 4:
                ...
                if (strlen(block) == 2) {
                    block_status = encrypt(sbox_filename, key_filename, pbox_filename, block, sub_keys);
                    if (BLOCK_KEY_OPEN_FILE_ERROR == block_status) printf("\n>>> Hubo un error al cargar la llave");
                    if (BLOCK_KEY_READ_ERROR == block_status) printf("\n>>> Hubo un erro al leer la llave");
                    if (BLOCK_SBOX_OPEN_FILE_ERROR == block_status) printf("\n>>> Hubo un error al cargar la S-Box");
                    if (BLOCK_PBOX_OPEN_FILE_ERROR == block_status) printf("\n>>> Hubo un error al cargar la P-Box");
                    if (BLOCK_PBOX_OUT_OF_THE_RANGE_ERROR == block_status) printf("\n>>> El tamano de la P-Box es incorrecto");
                    if (BLOCK_PBOX_REPEATED_VALUES_ERROR == block_status) printf("\n>>> Hay valores repetidos en la P-Box");
                } else {
            ...
            case 5:
                ...
                block_status = decrypt(sbox_filename, key_filename, pbox_filename, C, sub_keys);
                if (BLOCK_KEY_OPEN_FILE_ERROR == block_status) printf("\n>>> Hubo un error al cargar la llave");
                if (BLOCK_KEY_READ_ERROR == block_status) printf("\n>>> Hubo un erro al leer la llave");
                if (BLOCK_SBOX_OPEN_FILE_ERROR == block_status) printf("\n>>> Hubo un error al cargar la S-Box");
                if (BLOCK_PBOX_OPEN_FILE_ERROR == block_status) printf("\n>>> Hubo un error al cargar la P-Box");
