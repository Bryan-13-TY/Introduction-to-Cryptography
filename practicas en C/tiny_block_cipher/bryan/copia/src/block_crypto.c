...
BLOCKStatus secret_key_generator() {
    ...
    if (!fp) return BLOCK_KEY_GENERATION_ERROR;
 ...
 * 
 * @return
 * - BLOCK_OK si se cargo correctamente.
 * - BLOCK_KEY_OPEN_FILE_ERROR si hubo un error al abrir al archivo de la llave.
 * - BLOCK_KEY_READ_ERROR si hubo un error al leer la llave.
 */
static BLOCKStatus load_key(char key_filename[], unsigned short *K) {
    ...
    if (!fp) return BLOCK_KEY_OPEN_FILE_ERROR;

    if (fscanf(fp, "%X", K) != 1) {
        fclose(fp);
        return BLOCK_KEY_READ_ERROR;
    }
 ...
 * 
 * @return
 * - BLOCK_OK si se guardo correctamente.
 * - BLOCK_SBOX_GENERATION_ERROR si hubo un al generar y guardar la S-Box.
 */
static BLOCKStatus store_sbox(int size, unsigned char array[size], char textfile_name[]) {
    FILE *fp = fopen(textfile_name, "w");
    if (!fp) return BLOCK_SBOX_GENERATION_ERROR;
...
BLOCKStatus sbox_generator() {
    ...
    if (!array) return BLOCK_SBOX_MEMORY_ERROR;
    ...
    BLOCKStatus block_status = store_sbox(size_permutation, array, textfile_name);
 ...
 * @return
 * - BLOCK_OK si se cargo correctamente.
 * - BLOCK_SBOX_OPEN_FILE_ERROR si hubo un error al abrir el archivo de la S-Box.
 */
static BLOCKStatus load_sbox(char sbox_filename[], unsigned char sbox[]) {
    ...
    FILE *fp = fopen(full_path, "r");
    if (!fp) return BLOCK_SBOX_OPEN_FILE_ERROR;
 ...
 */
static BLOCKStatus store_pbox(int P[PBOX_SIZE], char textfile_name[]) {
    FILE *fp = fopen(textfile_name, "w");
    if (!fp) return BLOCK_PBOX_GENERATION_ERROR;
...
BLOCKStatus pbox_generator() {
    ...
    BLOCKStatus block_status = store_pbox(P, DATA_DIR "permutation.txt");
 ...
 * @return
 * - BLOCK_OK si se cargo correctamente.
 * - BLOCK_PBOX_OPEN_FILE_ERROR si hubo un error al abrir el archivo de la P-Box.
 ...
 */
static BLOCKStatus load_pbox(char pbox_filename[], int P[PBOX_SIZE]) {
    ...
    FILE *fp = fopen(full_path, "r");
    if (!fp) return BLOCK_PBOX_OPEN_FILE_ERROR;
    ...
    while (i < PBOX_SIZE && fscanf(fp, "%d", &value) == 1) {
        ...
        if (value < 1 || value > PBOX_SIZE) {
            fclose(fp);
            return BLOCK_PBOX_OUT_OF_THE_RANGE_ERROR;
        }

        // Repetidos
        if (seen[value - 1]) {
            fclose(fp);
            return BLOCK_PBOX_REPEATED_VALUES_ERROR;
        }
...
BLOCKStatus encrypt(
    ...
    block_status = load_key(key_filename, &K);
    if (BLOCK_OK != block_status) return block_status;

    block_status = load_sbox(sbox_filename, sbox);
    if (BLOCK_OK != block_status) return block_status;

    block_status = load_pbox(pbox_filename, pbox);
    if (BLOCK_OK != block_status) return block_status;
