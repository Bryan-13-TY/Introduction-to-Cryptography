...
unsigned short decryp_block(
    unsigned short C,
    unsigned short sub_keys[],
    unsigned char inv_sbox[SBOX_SIZE],
    int inv_pbox[PBOX_SIZE]
);
BlockCStatus encrypt_tcr(
    char sbox_filename[],
    char key_filename[],
    char pbox_filename[],
    char plaintext[],
...
int main(int argc, char const *argv[]) {
    int option = 0;
    char key_filename[100], sbox_filename[100], pbox_filename[100];
    ...
    do {
        ...
        switch (option) {
            ...
            case 4:
                ...
                blockc_status = encrypt_tcr(sbox_filename, key_filename, pbox_filename, plaintext, strlen(plaintext));
...
BlockCStatus encrypt_tcr(
    ...
    // Dividir en bloques
    int num_blocks = (size_plaintext + 1) / 2;
    unsigned short blocks[num_blocks];
    unsigned short ciphertext[num_blocks + 1];
    ...
    for (int i = 0; i < num_blocks; i++) {
        ...
        ciphertext[i + 1] = blocks[i] ^ keystream;
...
BlockCStatus decrypt_ctr(
    ...
    unsigned char C1 = 0;
    unsigned short plaintext[num_blocks];
